...
(async () => {
  ...
  launch("api", path.join(dir, "knowledge-api"), ["-f", configPath], {}, provider);
  const upstream = `http://127.0.0.1:${apiPort}`,
    base = `http://127.0.0.1:${webPort}`;
  await wait(`${upstream}/v1/knowledge/modules`);
  ...
  bootstrap = http.createServer((req, res) => {
    ...
    res.writeHead(302, {
      "Set-Cookie": `user_center_token=${token}; Path=/; SameSite=Lax; Max-Age=3600`,
