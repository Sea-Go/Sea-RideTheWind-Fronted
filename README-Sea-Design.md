...
本地 Chrome 已验证三主题切换及 390×844 手机端的新建对话、恢复历史、关闭与 Escape 焦点返回，并保存真实浏览器全页图。手机侧栏使用同一组处理器与状态；展开类名采用两个完整字符串，避免格式化器删掉插值前导空格。最终验证构建 ID 为
`mNyhX6CNQLZNhBRS8QbBi`。这些结果证明本地页面与交互，不代表新知识后端或真实模型服务已上线。
...
已增加真实管理员草稿列表、新建模块、原文/Wiki不可变修订、编制与构建命令以及带版本检查的人工发布接线。公开书架继续只展示服务端已发布模块。完整读面与历史回滚选择仍未完成，详细边界、复现和实测结果见[知识产品客户端](src/features/knowledge/README.md)。此阶段真实浏览器与 Next
BFF/go-zero/隔离 PostgreSQL 验收使用结构索引 fixture，不代表完整三路算法链路或上线验收。

## 知识读链补齐（2026-09-14）

知识工作台现已接入 RTW 的模块详情、逐修订正文和修订/候选/构建/编制历史分页。可读取已有正文进行编辑、比较，刷新后选择历史 READY 构建发布或回滚；公开目录与阅读绑定实际发布版本及修订，历史引用沿固定版本阅读原文。此前“正文/历史读面待接入”的阶段说明以[当前知识客户端文档](src/features/knowledge/README.md)为准。真实模型/三路索引质量及部署仍未验收。
